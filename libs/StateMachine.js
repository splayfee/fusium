...
import StateMachineError from "./StateMachineError.js";
import Transition from "./Transition.js";

/**
 * This class defines a new state machine which manages state for an application.
 */
export default class StateMachine {
   ...
   */
  transitionHandler(triggerId) {
    // Look for a local state transition
    let transition = this.currentState.getTransition(triggerId);
   ...
   */
  addTransition(triggerId, targetState) {
   ...
   * @returns {State}
   */
  createState(name, isAccept = false, entryAction = null, exitAction = null) {
    const id = this.states.length;
    const state = new State(this, id, name, isAccept);
   ...
   */
  trigger(triggerId, sendGlobal = false) {
    let finalTriggerId = triggerId;
    if (!sendGlobal) {
      finalTriggerId = `${triggerId}_state_${this.currentState.id.toString()}`;
    }

    this.transitionHandler(finalTriggerId);
  }
}